...
(defvar *format-dispatch-table* ()
  ;; initialized at the end of this module
...
(defun format-error (complaint &rest args)
   (throw
      'format-error
      (list
         (list "~1{~:}~%~S~%~V@T^" complaint args *format-control-string*
            (\1+ *format-index*)))) )
...
;;; the following are declared to get around the fact that unwind-protect
;;; as used above expands into an errorset, which loses lexical context.
(proclaim '(special control-string prev tilde))
...
(defmacro with-format-parameters (parmvar parmdefs &body forms)
   (do
      ((parmdefs parmdefs (cdr parmdefs))
         (bindings
            ()
            (cons
               `(,(caar parmdefs)
                  (or (if ,parmvar (pop ,parmvar)) ,(cadar parmdefs)))
               bindings)))
      ((null parmdefs)
         `(let*
            ,(nreverse bindings)
            (when ,parmvar (format-error "Too many parameters"))
            ,@forms))))
...
(defmacro nextchar ()
   '(if
      (< (incf *format-index*) *format-length*)
...
(defun format-find-command (command-list)
   (let
      ((start *format-index*))
      (do
         ((place start *format-index*)
            (tilde
               (format-find-char #\~ start *format-length*)
               (format-find-char #\~ place *format-length*)))
         ((not tilde) (format-error "Expecting one of ~S" command-list))
         (setq *format-index* tilde)
         (multiple-value-bind
            (parms colon atsign command)
            (parse-format-operation)
            (when
               (member command command-list :test #'char=)
...
(defun format (destination control-string &rest format-arguments)
   "Provides various facilities for formatting output.
     ...
     generally expand into additional text to be output, usually consuming
     one or more of the FORMAT-ARGUMENTS in the process.  A few useful
     directives are:
     ...
     then the output is sent to the standard output stream.  If it is NIL,
            ...
            (let
               ((errorp
                   (catch 'format-error
                      ...
                      nil)))
               (when errorp (format t "~%~:{~?~%~}" (nreverse errorp))
...
;;; and the appropriate handlers invoked with the arguments COLON, ATSIGN,
;;; and PARMS.
;;;
...
(defun sub-format (start end)
   (let
      ((*format-index* start) (*format-length* end))
      (do*
         ((place start *format-index*)
            (tilde
               (format-find-char #\~ start end)
               (format-find-char #\~ place end)))
         ((not tilde)
            (write-string
               *format-control-string* *standard-output* :start place :end
               end))
         (when
            (> tilde place)
            (write-string
...
(defun format-capitalization (colon atsign parms)
   (when parms (format-error "No parameters allowed to ~~("))
   (nextchar)
   (multiple-value-bind
      ...
      (let
         ((string (format-stringify-output (sub-format prev tilde))))
         (write-string
            (cond
               ((and atsign colon) (nstring-upcase string))
...
(defun format-escape (colon atsign parms)
   ...
   (when
      (if
         ...
         (if
            ...
            (if
               ...
               (typecase
                  (second parms)
                  (integer (<= (first parms) (second parms) (third parms)))
                  (character
      ...
      (if
         colon
         (throw 'format-colon-escape nil)
         (throw 'format-escape nil))))
      ...
      (do
         ...
         ((= count test)
            (multiple-value-bind
               ...
               (format-find-command '(#\; #\]))
               (when atsign (format-error "Atsign flag not allowed"))
               (when parms (format-error "No parameters allowed"))
               (sub-format prev tilde)
               (unless (char= cmd #\]) (format-find-command '(#\])))) )
         (multiple-value-bind
            (prev tilde parms colon atsign cmd)
            (format-find-command '(#\; #\]))
            (when atsign (format-error "Atsign flag not allowed"))
            (when parms (format-error "Parameters not allowed"))
      ...
      (if
         ...
         (multiple-value-bind
            ...
            (when
               ...
               (format-error "Flags or parameters not allowed"))
            (sub-format prev tilde))
         (progn (sub-format prev tilde) (format-find-command '(#\])))) ))
...
(defun format-condition (colon atsign parms)
   (when
      ...
      (unless (null parms) (format-error "Too many parameters to ~[")))
   (nextchar)
   (cond
      (colon
         (when atsign (format-error "~~:@[ undefined"))
            ...
            (do*
               ...
               (catch 'format-escape
                  (if
                     colon
                     (let*
                        ((*original-arguments* (pop-format-arg))
                           (*format-arguments* *original-arguments*))
                        (unless
                           (listp *format-arguments*)
...
(defun format-get-segments ()
   (multiple-value-bind
      ...
      (when atsign (format-error "Atsign flag not allowed"))
      (let
         ((first-seg (format-stringify-output (sub-format prev tilde))))
         (if
            (char= cmd #\;)
            (multiple-value-bind
               (segments numsegs numchars)
                  ...
                  (let
                     ((pos
                         (if
                            ...
                            0)))
                     (when
                        (> (+ pos width spare) linel)
                        (write-string special-arg)))) )
            (do
               ((segs segments (cdr segs)) (spcs spaces (cdr spcs)))
...
(defun format-page (colon atsign parms)
   ...
   (with-format-parameters
      parms
      ((repeat-count 1))
      (dotimes (i repeat-count) (write-char #.(special-char 5)))))
...
(defun format-newline (colon atsign parms)
   (when parms (format-error "Parameters not allowed"))
   (cond
      (colon (when atsign (format-error "~:@<newline> is undefined")))
...
(defun format-plural (colon atsign parms)
   ...
   (when
      ...
      (let
         ((cdrs
             (-
                (length *format-original-arguments*)
            ...
            (let
               ...
               (if
                  ...
                  (setq *format-arguments*
                     (nthcdr cdrs *format-original-arguments*)))) )
         (t (if
               (> count (length *format-arguments*))
               (format-error "Skip to nonexistant argument")
               (setq *format-arguments*
                  (nthcdr count *format-arguments*)))) )))
...
;;; brought into line with Common Lisp manual Feb 1986. ACN

(defun format-tab (colon atsign parms)
   (with-format-parameters
      parms
      ((colnum 1) (colinc 1))
      (when colon (format-error "':' flag in '~T' not supported"))
      (write-string
"                                                                                " *standard-output* :start 0 :end
         (let
            ((position (charpos *standard-output*)))
            (cond
               (position
                  (if atsign
                     (let*
                        ((pad (rem (+ position colnum) colinc))
                         (pad1 (if (= pad 0) 0 (- colinc pad))))
                        (min
                           (+ colnum pad1)
                           80))
                     (min
                        (if
                           (> position colnum)
                           (- colinc (rem (- position colnum) colinc))
                           (- colnum position))
                        80)))
               (atsign colnum)
               (t 2)))) ))
   ...
   (let
      ...
      (if
         ...
         (with-format-parameters
            ...
            ((mincol 0) (colinc 1) (minpad 0) (padchar #\ ))
            (format-write-field
               (if arg (princ-to-string arg) (if colon "()" "NIL"))
               mincol
               colinc
               minpad
               padchar
   ...
   (let
      ...
      (if
         ...
         (with-format-parameters
            ...
            ((mincol 0) (colinc 1) (minpad 0) (padchar #\ ))
            (format-write-field
               (if arg (prin1-to-string arg) (if colon "()" "NIL"))
               mincol
               colinc
               minpad
               padchar
...
(defun format-print-character (colon atsign parms)
   (with-format-parameters
      ...
      (let
         ...
         (cond
            ((and atsign (not colon)) (prin1 char))
            ;;;????? write-char ??
...
(defun format-print-named-character (char longp)
   (write-string
      (cond
         ((char-bit char :control) (if longp "Control-" "C-"))
         ((char-bit char :meta) (if longp "Meta-" "M-"))
         ((char-bit char :super) (if longp "Super-" "S-"))
         ((char-bit char :hyper) (if longp "Hyper-" "H-"))
         (t "")))
   (let*
      ((ch (code-char (char-code char)))
         ;strip funny bits
         (name (char-name ch)))
      (cond
...
(defun format-add-commas (string commachar)
   (do*
      ((length (length (the string string)))
         (new-length (+ length (floor (\1- length) 3)))
         (new-string
            (make-string new-length :initial-element commachar)
            (replace
               (the string new-string)
               (the string string)
               :start1 (max 0 (- new-pos 3)) :end1 new-pos :start2
               (max 0 (- pos 3)) :end2 pos))
         (pos length (- pos 3))
...
(defun format-write-field (string mincol colinc minpad padchar padleft)
   ...
   (unless
      ...
      (format-error "Padchar must be a character - ~S" padchar))
   (let*
      ((strlen (length (the string string)))
...
(defun format-print-number (number radix print-commas-p print-sign-p parms)
   (with-format-parameters
      parms
      ((mincol 0) (padchar #\ ) (commachar #\,))
      (let*
         ((*print-base* radix) (text (princ-to-string number)))
         (if
            (integerp number)
            (format-write-field
               (if
                  (and (plusp number) print-sign-p)
                  (if
                     print-commas-p
                     (concatenate
                        'string
...
(defvar cardinal-teens
   '#.(vector "ten" "eleven" "twelve" "thirteen" "fourteen" ;;; RAD
      ...
      (when
         ...
         (multiple-value-bind
            ...
            (cond
               ...
               ((= tens 1) (write-string (svref cardinal-teens ones)))
               ((plusp ones)
                  (write-string (svref cardinal-ones ones)))) ))) )
...
(defun format-print-cardinal-aux (n period err)
   (multiple-value-bind
      ...
      (unless
         (zerop here)
         (unless (zerop beyond) (write-char #\ ))
...
(defvar ordinal-ones
   '#.(vector nil "first" "second" "third" "fourth" "fifth" "sixth" "seventh"
           "eighth" "ninth")
   "Table of ordinal ones-place digits in English")

(defvar ordinal-tens
   '#.(vector nil "tenth" "twentieth" "thirtieth" "fortieth" "fiftieth" "sixtieth"
          "seventieth" "eightieth" "ninetieth")
   "Table of ordinal tens-place digits in English")


(defun format-print-ordinal (n)
   (when (minusp n) (write-string "negative "))
   (let
      ((number (abs n)))
      (multiple-value-bind
         (top bot)
         ...
         (when (and (plusp top) (plusp bot)) (write-char #\ ))
         (multiple-value-bind
            (tens ones)
            (truncate bot 10)
            (cond
               ((= bot 12) (write-string "twelfth"))
               ((= tens 1)
...
(defun format-print-old-roman (n)
   (unless
      (< 0 n 5000)
      (format-error
         "Number out of range for printing in old Roman numerals: ~:D"
         n))
   (do
      ((char-list '(#\D #\C #\L #\X #\V #\I) (cdr char-list))
         ...
         (start
            n
            (do
               ((i start (progn (write-char cur-char) (- i cur-val))))
               ((< i cur-val) i))))
      ((zerop start))))
...
(defun format-print-roman (n)
   (unless
      (< 0 n 4000)
      (format-error
         "Number out of range for printing in Roman numerals: ~:D"
         n))
   (do
      ((char-list '(#\D #\C #\L #\X #\V #\I) (cdr char-list))
         (val-list '(500 100 50 10 5 1) (cdr val-list))
...
(defun format-fixed (colon atsign parms)
   (when colon (format-error "Colon flag not allowed"))
   (with-format-parameters
      parms
      ((w nil) (d nil) (k nil) (ovf nil) (pad #\ ))
      ...
      (let
         ...
         (if
            ...
            (if
               (rationalp number)
               (format-fixed-aux (coerce number 'short-float) w d k ovf pad
                  atsign)
               (let
                  ((*print-base* 10))
...
(defun format-exponential (colon atsign parms)
   ...
   (with-format-parameters
      parms
      ((w nil) (d nil) (e 2) (k 1) (ovf nil) (pad #\ ) (marker nil))
      (let
         ((number (pop-format-arg)))
         (if
            (floatp number)
            ...
            (if
               (rationalp number)
               (format-exp-aux (coerce number 'short-float) w d e k ovf pad
                  marker atsign)
               (let
                  ((*print-base* 10))
                  (format-write-field (princ-to-string number)
...
(defun format-exponent-marker (number)
   (if
      (typep number *read-default-float-format*)
      #\E
      (typecase
         number
         (short-float #\S)
...
;;;to the left right, athough it is an error to specify k and d such that
;;;this occurs.  Perhaps we should detect both these condtions and flag
;;;them as errors.  As for now, we let the user get away with it, and
;;;merely guarantee that at least one significant digit will appear.
...
(defun format-general-float (colon atsign parms)
   ...
   (with-format-parameters
      parms
      ((w nil) (d nil) (e nil) (k nil) (ovf #\*) (pad #\ ) (marker nil))
      (let
         ((number (pop-format-arg)))
         ...
         (if
            (floatp number)
            (if (float-infinity-p number)
               (format-exp-aux number w d e k ovf pad marker atsign)
               (format-general-aux number w d e k ovf pad marker atsign))
            (if
               (rationalp number)
               (format-general-aux (coerce number 'short-float) w d e k ovf
                  pad marker atsign)
               (let
                  ((*print-base* 10))
                  (format-write-field (princ-to-string number)
      ...
      (let*
         ...
         (cond
            ((<= 0 dd d)
               ...
               (dotimes (i ee) (write-char #\ )))
            (t (format-exp-aux number w d (or e 2) (or k 1) ovf pad marker
                  atsign)))) ))
...
;;; lists (<character> <function-object>) and returns a vector
;;; char-code-limit elements in length, where the Ith element is the
;;; function associated with the character with char-code I.  If the
;;; character is case-convertible, it must be given in only one case;
;;; however, an entry in the vector will be made for both.

;;; The only reason this is a vector rather than a function is so that
;;; it can expand into the (vector ...) construct to build the object
;;; finally wanted. This seems marginally easier than doing a list to
;;; vector conversion any other way.


(defmacro make-dispatch-vector (&body entries)
;; Note that I can not generate just (vector ...) since that is only
;; entitled to create vectors of length up to 50, and here I need one of
;; length 256.
   (let
      ((entries
          (mapcan
             #'(lambda (x)
                (let
                   ((lower (char-downcase (car x)))
                      (upper (char-upcase (car x))))
                   (if
                      (char= lower upper)
                      (list x)
                      (list (cons upper (cdr x)) (cons lower (cdr x)))) ))
             entries)))
      (do
         ((entries (sort entries #'(lambda (x y) (char< (car x) (car y)))) )
            (charidx 0 (\1+ charidx))
            (comtab ()
               (cons
                  (if
                     entries
                     (if
                        (= (char-code (caar entries)) charidx)
                        (eval (cadr (pop entries)))
                        nil)
                     nil)
                  comtab)))
         ((= charidx char-code-limit)
            (if entries (error "Garbage in dispatch vector - ~S" entries))
            `(list-to-vector ',(nreverse comtab)))) ))
...
(setq *format-dispatch-table*
      (make-dispatch-vector
         ...
         (#.newline-char #'format-newline)))


(defun warn (control-string &rest format-arguments)
   (let
      ((*format-original-arguments* format-arguments)
       (*format-arguments* format-arguments)
       (*format-control-string* (coerce control-string 'simple-string)))
